export const AES = {
  ...
  cipher: function (input, w) {
    var nb = 4; //kích thước khối, 4word=4 cột;hàng
    var nr = w.length / nb - 1; //số lượng vòng lặp
    var state = [new Array(nb), new Array(nb), new Array(nb), new Array(nb)];
    var output = new Array(4 * nb);

    var i, round;
    //chuyển đổi dữ liệu đầu vào thành state
    for (i = 0; i < input.length; i++) {
      state[i % 4][Math.floor(i / 4)] = input[i];
      //ma trận 2 chiều cột-hàng(chia lấy dư và chia lấy phần nguyên)//vì chỉ có phép
      //ccootj từ 0-3, hàng thì vô hạn?
    }
